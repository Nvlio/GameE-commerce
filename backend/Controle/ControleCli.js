...
export default class ClienteControle {
    ...
    async POST(req, resp) {
        if (req.method == "POST" && req.is('application/json')) {
            ...
            if (cpf, nome, telefone, senha, email, endereco || (email,senha) && tipo==="Login") {
                const modelo = new ClienteMod(cpf, nome, telefone, senha, email, endereco);
    ...
    async PUT(req, resp) {
        if (req.method == "PUT" && req.is('application/json')) {
            ...
            if (cpf != undefined || cpf != "" || cpf != "undefined") {
                const modelo = new ClienteMod(cpf);
                const resposta = await modelo.Atualizar(nome, telefone, senha, email, endereco)
